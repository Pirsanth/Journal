...
module.exports = function (req, username, month, year, res) {
    ...
    validateSession(req, function (err, isValid, sessionUsername) {
      ...
      if(isValid){
        ...
        /*Not valid if either there is no sessionId cookie with the request, it has been tampered with or the id has expired (no document
        ...
        res.writeHead(302, {"Location": "../login.html", "Set-Cookie": ["errorMessage=Not logged in or session has expired please login to view resource; Path=/login.html; HttpOnly", "sessionId=; Path=/; HttpOnly", "digest=; Path=/; HttpOnly"]});
        res.end();
      }
    });
